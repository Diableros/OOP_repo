class VehicleFlyweight {
  ...
  public render(x: number, y: number, direction: Direction) {
    console.log(
      `Rendered ${this.vehicleType} in position {${x}, ${y}} with direction ${direction}º`,
    );
  }
}
...
class VehicleFactory {
  private static vehicleFlyweights = new Map<VehicleType, VehicleFlyweight>();
  ...
  protected static getVehicle(
    ...
    if (!this.vehicleFlyweights.has(vehicleType)) {
      this.vehicleFlyweights.set(
        vehicleType,
        new VehicleFlyweight(vehicleType),
      );
    }
    return new Vehicle(
      vehicleType,
...
console.log(
  `After creating ${vehicles.length} vehicles the application takes:`,
);
for (const [key, value] of Object.entries(process.memoryUsage())) {
  console.log(`    ${Math.round(value / (1024 * 1024))}MB of ${key}`);
}
...
console.log(
  `After creating ${flyweights.length} flyweights finally the application takes:`,
);
for (const [key, value] of Object.entries(process.memoryUsage())) {
  console.log(`    ${Math.round(value / (1024 * 1024))}MB of ${key}`);
}
