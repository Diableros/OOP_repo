...
import {
  ...
  Flight,
  Passenger
} from '@/pages/Prototype/models/registry.ts'
import React from 'react'

const PrototypeRegistry = () => {
  const [form] = Form.useForm<Airport>()

  const [airport, setAirport] = React.useState<Airport>(new Airport())
  const saveCloneFlight = (flight: number) => {
    const flights: Flight[] = form.getFieldValue('flights')
    setAirport(airport.add(flights[flight].clone()))
  }
        ...
        {(fields, { add: addFlight, remove: removeFlight }) => (
          <Card title={<TwitterOutlined />}>
            <Space direction='vertical'>
              <Space>
                {Object.entries(airport.prototypes).map(([id, flight]) => (
                  <Button key={id} onClick={() => addFlight(flight)}>
                    №{Number(id) + 1} {flight.name}
                  </Button>
                ))}
              </Space>
              {fields.map(
                ({ key: keyFlight, name: nameFlight, ...restField }) => (
                  ...
                  >
                              ...
                              ({
                                ...
                                >
                                  ...
                                  <Card title={<ShoppingOutlined />}>
                                    <Form.List
                                      name={[namePassenger, 'baggage']}
                                    >
                                      {(
                                        fields,
                                        {
                                          add: addBaggage,
                                          remove: removeBaggage
                                        }
                                      ) => (
                                        <>
                                          {fields.map(
                    ...
                    />
                    <SaveOutlined onClick={() => saveCloneFlight(nameFlight)} />
